...
// ── Jersey kits per team — using kit SVG illustrations via api.football-data ──
// Source: Wikimedia Commons kit diagrams (CC license, reliable CDN)
const TEAM_JERSEYS = {
...
async function fetchJerseyImage(teamId, kitType, teamName, teamEn) {
  ...
  try {
    const prompt = `You are a sports kit image generator. Generate an accurate visual description then return a base64 PNG of a ${teamEn} national football team ${kitLabels[kitType]} jersey (shirt only, flat lay, no player, white background). The jersey should be ${kit.label} colored (${kit.color}) with ${kit.accent} accents/trim. Make it photorealistic and detailed, showing the actual 2026 World Cup kit design. Return ONLY a JSON object: {"svg": "<svg...complete SVG code of a realistic detailed football jersey...>"}`;
...
Style: ${kit.label}
${teamId === "croatia" && kitType === "home" ? "Pattern: red and white checkerboard squares (šahovnica)" : ""}
${teamId === "argentina" && kitType === "home" ? "Pattern: light blue and white vertical stripes" : ""}
${teamId === "germany" && kitType === "home" ? "Style: clean white with subtle pinstripes, black collar" : ""}
${teamId === "brazil" && kitType === "home" ? "Style: canary yellow with green collar and green trim" : ""}
${teamId === "france" && kitType === "home" ? "Style: deep navy blue, inspired by Statue of Liberty" : ""}
${teamId === "netherlands" && kitType === "home" ? "Style: bold total orange, minimalist" : ""}

Return ONLY a valid SVG (viewBox="0 0 200 220") of just the jersey shirt, front view, no player, detailed fabric texture and realistic kit design. Make it look like a real football jersey product photo.` }]
      })
    });
...
function JerseyCard({ teamId, kitType, teamName, teamEn, size = 110, showName = "", showNumber = "" }) {
  ...
  const FallbackSVG = () => {
    ...
    return (
      <svg width={size} height={size*1.1} viewBox="0 0 200 220" xmlns="http://www.w3.org/2000/svg"
        ...
        <defs>
          {isChecker && (
            <pattern id={`chk_${kitType}`} x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
              <rect width="10" height="10" fill={kit.color}/>
              <rect x="10" width="10" height="10" fill={kit.accent}/>
              <rect y="10" width="10" height="10" fill={kit.accent}/>
              <rect x="10" y="10" width="10" height="10" fill={kit.color}/>
            </pattern>
          )}
          {isStripes && (
            <pattern id={`str_${kitType}`} x="0" y="0" width="24" height="1" patternUnits="userSpaceOnUse">
              <rect width="14" height="220" fill={kit.color}/>
              <rect x="14" width="10" height="220" fill={kit.accent}/>
            </pattern>
          )}
          {isPinstripe && (
            <pattern id={`pin_${kitType}`} x="0" y="0" width="12" height="1" patternUnits="userSpaceOnUse">
              <rect width="10" height="220" fill={kit.color}/>
              <rect x="10" width="2" height="220" fill={kit.accent} opacity="0.3"/>
            </pattern>
          )}
          <filter id={`shadow_${kitType}`}>
            <feDropShadow dx="2" dy="4" stdDeviation="3" floodOpacity="0.4"/>
          </filter>
          <linearGradient id={`shine_${kitType}`} x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        {/* Left sleeve */}
        <path d="M20 38 L8 110 L62 118 L70 40 Z"
          fill={isChecker?`url(#chk_${kitType})`:isStripes?`url(#str_${kitType})`:isPinstripe?`url(#pin_${kitType})`:kit.color}
          stroke={kit.accent} strokeWidth="2" filter={`url(#shadow_${kitType})`}/>
        {/* Right sleeve */}
        <path d="M180 38 L192 110 L138 118 L130 40 Z"
          fill={isChecker?`url(#chk_${kitType})`:isStripes?`url(#str_${kitType})`:isPinstripe?`url(#pin_${kitType})`:kit.color}
          stroke={kit.accent} strokeWidth="2"/>

        {/* Body */}
        <path d="M70 40 L62 118 L55 205 L145 205 L138 118 L130 40 Q100 58 70 40 Z"
          fill={isChecker?`url(#chk_${kitType})`:isStripes?`url(#str_${kitType})`:isPinstripe?`url(#pin_${kitType})`:kit.color}
          stroke={kit.accent} strokeWidth="2" filter={`url(#shadow_${kitType})`}/>

        {/* Shine overlay */}
        <path d="M70 40 L62 118 L55 205 L145 205 L138 118 L130 40 Q100 58 70 40 Z"
          fill={`url(#shine_${kitType})`}/>
        ...
        {/* Number */}
        {showNumber && (
          <text x="100" y="165" textAnchor="middle" fontSize="44" fontWeight="900"
            fill={kit.accent} fontFamily="'Bebas Neue',Arial,sans-serif"
        ...
        {/* Name */}
        {showName && (
          <text x="100" y="130" textAnchor="middle" fontSize="13" fontWeight="700"
            fill={kit.accent} fontFamily="Cairo,Arial,sans-serif" letterSpacing="2">
            {showName.slice(0,9).toUpperCase()}
          </text>
        )}
  ...
  if (loading) return (
    <div style={{width:size,height:size*1.1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8}}>
      <div style={{fontSize:"1.6rem",animation:"spin 1s linear infinite"}}>⟳</div>
      <div style={{fontSize:"0.6rem",color:"rgba(255,255,255,0.4)"}}>جاري التحميل...</div>
    </div>
  );

  if (imgData?.type === "svg") {
    return (
      <div style={{width:size,height:size*1.1,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
        <div dangerouslySetInnerHTML={{__html: imgData.data}}
          style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center"}}/>
        {(showName||showNumber) && (
          <div style={{position:"absolute",bottom:"14%",left:0,right:0,textAlign:"center"}}>
            {showName && <div style={{fontSize:"0.55rem",fontWeight:700,color:kit.accent,letterSpacing:1,opacity:0.9}}>{showName.slice(0,9).toUpperCase()}</div>}
            {showNumber && <div style={{fontSize:"1.4rem",fontWeight:900,color:kit.accent,fontFamily:"'Bebas Neue',sans-serif",lineHeight:1}}>{showNumber}</div>}
          </div>
        )}
      </div>
    );
...
// ── Countdown Timer ───────────────────────────────────────────────────────────
// Registration closes: June 11, 2026 at 18:00 Riyadh time (UTC+3) — kickoff
const REGISTRATION_DEADLINE = new Date("2026-06-11T15:00:00Z"); // 18:00 Riyadh = 15:00 UTC
...
function CountdownBanner() {
  ...
  if (!t) return (
    <div style={{background:"rgba(255,60,60,0.15)",border:"1px solid rgba(255,60,60,0.4)",borderRadius:12,padding:"10px 16px",textAlign:"center",marginBottom:16,fontSize:"0.85rem",color:"#ff8888"}}>
      🔒 انتهى وقت التسجيل — البطولة انطلقت!
    </div>
  );

  const urgent = t.diff < 86400000; // less than 1 day
  return (
    <div style={{
      background: urgent ? "rgba(255,60,60,0.12)" : "rgba(212,175,55,0.1)",
      border: `1px solid ${urgent?"rgba(255,60,60,0.4)":"rgba(212,175,55,0.3)"}`,
      borderRadius:12, padding:"12px 16px", marginBottom:16, textAlign:"center"
    }}>
      <div style={{fontSize:"0.72rem",color:"var(--muted)",marginBottom:6,letterSpacing:1}}>
        ⏱ يُغلق التسجيل عند انطلاق أول مباراة
      </div>
        ...
        {[{v:t.d,l:"يوم"},{v:t.h,l:"ساعة"},{v:t.m,l:"دقيقة"},{v:t.s,l:"ثانية"}].map(({v,l})=>(
          <div key={l} style={{textAlign:"center"}}>
            <div style={{
              ...
              fontSize: urgent ? "2rem":"1.7rem",
              color: urgent ? "#ff6666":"var(--gold)",
              lineHeight:1,
              textShadow: urgent ? "0 0 16px rgba(255,80,80,0.5)":"0 0 12px rgba(212,175,55,0.5)",
              minWidth:42,
              background:"rgba(0,0,0,0.25)",
              borderRadius:8, padding:"4px 6px",
              border:`1px solid ${urgent?"rgba(255,60,60,0.3)":"rgba(212,175,55,0.2)"}`
            }}>
...
function probColor(p) {
  if (p >= 12) return "#4cff88";
  if (p >= 6)  return "#44ddff";
  if (p >= 2)  return "#ffcc44";
  return "#ff8866";
}
function barGrad(p) {
  if (p >= 12) return "linear-gradient(90deg,#4cff88,#00cc66)";
  if (p >= 6)  return "linear-gradient(90deg,#44ddff,#0099cc)";
  if (p >= 2)  return "linear-gradient(90deg,#ffcc44,#cc8800)";
  return "linear-gradient(90deg,#ff8866,#cc4422)";
}
function tierClass(p) {
  if (p >= 12) return "tier-top";
  if (p >= 6)  return "tier-high";
  if (p >= 2)  return "tier-mid";
  return "tier-low";
}
...
export default function App() {
  ...
  const [odds, setOdds] = useState(() => {
    const o = {}; TEAMS.forEach(t => { o[t.id] = t.odds; }); return o;
  });
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(loadParticipants, 15000);
  ...
  const handleRegister = async () => {
    ...
    const row = {
      ...
      team_name: team.name,
      team_flag: team.flag,
      jersey: { name: regJerseyName.trim(), number: regJerseyNum.trim(), color: regJerseyColor, size: regJerseySize },
      photo: regPhoto,
      vote_changed: false,
  ...
  const handleUpdateOdds = useCallback(async () => {
    ...
    if (fresh) {
      ...
      showToast("⚠️ تعذّر التحديث — آخر بيانات محفوظة");
    }
  }, []);
  ...
  const handleVoteChange = async () => {
    ...
    if (teamCount(voteChangeTeam) >= MAX_PER_TEAM)
      return showToast(`⚠️ هذا المنتخب وصل الحد الأقصى`);
      ...
      <div className="pitch-bg">
        ...
        <div className="hdr">
          ...
          <div style={{display:"flex",justifyContent:"center"}}>
            <div className="hdr-live"><span className="live-dot"/>بيانات مشتركة • تحديث فوري</div>
          </div>
        </div>
            ...
            <div className="card">
              ...
              {regStep === 1 && (
                <>
                  ...
                  {!termsAccepted&&(
                    <div style={{textAlign:"center",marginTop:10,fontSize:"0.75rem",color:"#ffaa44"}}>
                      ⚠️ لم توافق على الشروط بعد —{" "}
                      <span style={{color:"var(--gold)",cursor:"pointer",textDecoration:"underline"}} onClick={()=>setTab("terms")}>اقرأها هنا</span>
                    </div>
                  )}
                </>
              )}
              ...
              {regStep === 2 && (
                <>
                  ...
                  {regTeam&&(
                    <div style={{textAlign:"center",marginBottom:10,fontSize:"0.86rem",color:"var(--gold)"}}>
                      ✓ اخترت: {TEAMS.find(t=>t.id===regTeam)?.flag} {TEAMS.find(t=>t.id===regTeam)?.name}
                    </div>
                  )}
                ...
                <>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:18}}>
                    {JERSEY_KIT_TYPES.map(kit => (
                      <div key={kit.id}
                        onClick={()=>setRegJerseyColor(kit.id)}
                        style={{
                          border: regJerseyColor===kit.id ? "2px solid var(--gold)" : "2px solid rgba(255,255,255,0.1)",
                          borderRadius:14, padding:"12px 6px 8px",
                          background: regJerseyColor===kit.id ? "rgba(212,175,55,0.12)" : "rgba(255,255,255,0.04)",
                          cursor:"pointer", textAlign:"center",
                          boxShadow: regJerseyColor===kit.id ? "0 0 16px rgba(212,175,55,0.3)" : "none",
                          transition:"all 0.18s", position:"relative"
                        }}>
                        {regJerseyColor===kit.id && (
                          <div style={{position:"absolute",top:6,left:8,fontSize:"0.65rem",color:"var(--gold)",fontWeight:900}}>✓</div>
                        )}
                        <JerseyCard
                          teamId={regTeam}
                          kitType={kit.id}
                          teamName={TEAMS.find(t=>t.id===regTeam)?.name}
                          teamEn={TEAMS.find(t=>t.id===regTeam)?.en}
                          size={90}
                          showName={regJerseyName}
                          showNumber={regJerseyNum}
                        />
                        <div style={{fontSize:"0.72rem",fontWeight:700,color:regJerseyColor===kit.id?"var(--gold)":"var(--muted)",marginTop:4}}>
                          {kit.label}
                        </div>
                      </div>
                    ))}
                    ...
                    {JERSEY_SIZES.map(s=>(
                      <button key={s} onClick={()=>setRegJerseySize(s)}
                        style={{
                          ...
                          border: regJerseySize===s ? "2px solid var(--gold)" : "1.5px solid rgba(255,255,255,0.15)",
                          background: regJerseySize===s ? "rgba(212,175,55,0.18)" : "rgba(255,255,255,0.05)",
                          color: regJerseySize===s ? "var(--gold)" : "var(--muted)",
                  ...
                  <div style={{border:"1.5px dashed rgba(212,175,55,.3)",borderRadius:10,padding:"14px",textAlign:"center",marginBottom:14,cursor:"pointer",fontSize:"0.82rem",color:"var(--muted)"}}
            ...
            <>
              ...
              {voteChangeTarget&&(
                <div className="card" style={{marginBottom:14}}>
                  ...
                  <div style={{background:"rgba(68,221,255,.07)",border:"1px solid rgba(68,221,255,.25)",borderRadius:10,padding:"10px 12px",marginBottom:12,fontSize:"0.8rem",color:"#a0e8ff"}}>
                    متاح مرة واحدة فقط بعد انتهاء دور المجموعات
                  </div>
                  <input className="inp" placeholder="🔍 ابحث..." value={searchTeam}
                    onChange={e=>setSearchTeam(e.target.value)}
                    style={{marginBottom:10,fontSize:"0.84rem",padding:"9px 13px"}} />
                  <div className="team-grid" style={{maxHeight:200}}>
                    {filteredTeams.map(team=>{
                      const p=odds[team.id]||team.odds, cnt=teamCount(team.id), full=cnt>=MAX_PER_TEAM;
                      return (
                        <div key={team.id} className={`t-tile ${tierClass(p)}${voteChangeTeam===team.id?" sel-t":""}${full?" full":""}`}
                          onClick={()=>!full&&setVoteChangeTeam(team.id)}>
                          <span className="t-flag">{team.flag}</span>
                          <div className="t-name">{team.name}</div>
                          <div className="t-odds">~{p}%</div>
                        </div>
                  ...
                  : leaderboard.map((p,i)=>{
                      ...
                      return (
                        <div key={p.id} className={`p-card ${rc}`} onClick={()=>openModal(p)}>
                          ...
                          {p.photo
                            ? <img src={p.photo} style={{width:42,height:42,borderRadius:"50%",objectFit:"cover",border:"2px solid var(--gold)",flexShrink:0}} alt=""/>
                            : <div className="p-flag">{p.team_flag}</div>
                          }
                          ...
                          {p.can_change_vote&&!p.vote_changed&&(
                            <button className="btn btn-sm btn-outline"
                              style={{fontSize:"0.68rem",padding:"5px 8px",width:"auto",minWidth:0}}
                              onClick={e=>{e.stopPropagation();setVoteChangeTarget(p.id);setVoteChangeTeam(null);}}>🔄</button>
                          )}
                          <div className="p-bar-wrap">
                            <div className="p-bar" style={{width:`${Math.round((prob/topProb)*100)}%`,background:barGrad(prob)}}/>
                          </div>
                        </div>
                      );
                    })
              ...
              {!loading&&leaderboard.length>0&&(
                <div style={{textAlign:"center",marginTop:14,fontSize:"0.73rem",color:"var(--muted)"}}>
                  {leaderboard.length} مشارك • اضغط على أي بطاقة لرؤية بيانات التيشرت
                </div>
              )}
            </>
          )}
                ...
                ? (
                  ...
                  <>
                        ...
                        : participants.map(p=>(
                          <div key={p.id} className="a-row">
                            ...
                            <button style={{background:"none",border:"none",color:"#4cff88",cursor:"pointer",fontSize:"0.76rem",fontFamily:"Cairo,sans-serif",padding:"2px 5px"}}
                              onClick={async()=>{
                                await sb.update(p.id,{can_change_vote:true,vote_changed:false});
                                await loadParticipants();
                                showToast("✅ تم تفعيل تغيير التصويت");
                              }}>🔓</button>
                            <button className="btn btn-danger btn-sm"
                              onClick={async()=>{
                                await sb.delete(p.id);
                    ...
                    <div className="card">
                      ...
                      <button className="btn" style={{marginBottom:8,background:"rgba(76,255,136,.15)",border:"1px solid rgba(76,255,136,.3)",color:"#4cff88",boxShadow:"none"}}
                        onClick={loadParticipants}>🔄 تحديث القائمة</button>
                      <button className="btn btn-danger"
                        onClick={async()=>{
                          if(window.confirm("⚠️ حذف جميع المشاركين؟ لا يمكن التراجع.")){
                            await sb.deleteAll();
                            setParticipants([]);
                            showToast("تم حذف جميع المشاركين");
                          }
                        }}>🗑️ إعادة تعيين المسابقة</button>
                    </div>
                    <div style={{textAlign:"center",marginTop:8}}>
                      <button style={{background:"none",border:"none",color:"var(--muted)",fontSize:"0.76rem",cursor:"pointer",fontFamily:"Cairo,sans-serif"}}
                        onClick={()=>setAdminUnlocked(false)}>تسجيل خروج</button>
                    </div>
                  </>
                )
              }
          ...
          <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setModalP(null)}>
            <div className="modal">
              ...
              <div className="jersey-preview">
                {modalP.photo
                  ? <img src={modalP.photo} style={{width:80,height:80,borderRadius:"50%",objectFit:"cover",border:"3px solid var(--gold)"}} alt=""/>
                  : null
                }
                <div style={{display:"flex",justifyContent:"center",marginTop: modalP.photo ? 10 : 0}}>
                  <JerseyCard
                    teamId={modalP.team_id}
                    kitType={modalP.jersey?.color || "home"}
                    teamName={modalP.team_name}
                    teamEn={TEAMS.find(t=>t.id===modalP.team_id)?.en || modalP.team_name}
                    size={130}
                    showName={modalP.jersey?.name}
                    showNumber={modalP.jersey?.number}
                  />
                </div>
              ...
              <button className="btn" style={{marginTop:14}} onClick={()=>setModalP(null)}>إغلاق</button>
            </div>
          </div>
        )}
      </div>
